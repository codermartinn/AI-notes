...
- 【ChatGPT 提示工程师&AI 大神吴恩达教你写提示词｜ prompt engineering【完整中字九集全】】 https://www.bilibili.com/video/BV1Z14y1Z7LJ/?share_source=copy_web&vd_source=8be40ea3f12a47b7649f7dd768ae4f24
- 参考笔记：
...
- Give the model time to think 给 chatgpt 一点时间去想
...
下一个策略是要求结构化输出。因此，为了使解析模型输出更容易，要求结构化输出（如超文本标记语言或 JSON）可能会有所帮助。让我复制另一个例子。所以在提示符中，我们说生成三个虚构书名的列表，以及它们的作者和流派，以 JSON 格式提供以下键，图书 ID，标题，作者和流派。
...
如您所见，我们有三个虚构的书名格式化在这个漂亮的 JSON 结构化输出中。
...
这件事的好处是，您实际上可以在 Python 中将其读入字典或列表。
...
> 就是 AI 举个对话的例子，然后 AI 会按照你的例子继续编。
...
第四，输出一个 JSON 对象，包含以下键：法语摘要和名称数。
...
学生的方案是`100x + 250x + 100,000 + 100x = 450x + 100,000`，明显是错的。但是 GPT 会认为是对的。模型只是按照我的思考方式匆匆看
...
我们可以通过让模型先计算自己的解决方案,然后，在要求比较这个答案和学生的答案时,让 AI 意识到它们不一致，
...
展示一个例子，在这个例子中模型会产生幻觉。这是一个例子，展示了模型如何编造一个来自真实牙刷公司的虚构产品名称的描述。因此，这个提示是：“告诉我关于 Boy 的 AeroGlide Ultra Slim 智能牙刷的情况。”如果我们运行它，模型将为我们提供一个相当逼真的虚构产品的描述。
